...
// Todo : Show stck traces for failures
...
var TinyTest = {
    ...
    run: function(tests) {
        var failures = 0;
        for (var testName in tests) {  // testName is the different methods to test
            var testAction = tests[testName];
            try {
                testAction.apply(this);  // bind and run
        ...
        setTimeout(function() { // Give document a chance to complete
            if (window.document && document.body) {
                ...
                TinyTestsHelper.renderStats(tests, failures);

            }
        }, 0);  // timeOut is immediate, setTimeout is used to delay the callback after 1. and 2.
                // gives the color if failure of success
